...
using System;
public class review_ins_list_controller : MonoBehaviour
{
    ...
    void RTchngr()
    {
        RT_review.changeScript(RTgetter());
    }
    List<string> RTgetter()
    {
        string sqlQuery = "Select RT from " + relats_review.tableName + ";";
        if (Language_Manager.lang == 1) sqlQuery = "Select RTF from " + relats_review.tableName + ";";
        return (relats_review.sqliteExecuter(sqlQuery));
    }
