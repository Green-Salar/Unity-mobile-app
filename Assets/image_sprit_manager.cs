...
public class image_sprit_manager : MonoBehaviour
{
    ...
    public void Load_Image_review(string _img_name_str, int imgNum)// imgnums 1-7 imgs. 1-6 for report and 7th is for showing the img clicked
    {
        if (_img_name_str != "-")
        {
            try
            {
                ...
                //mg[imgNum].gameObject.AddComponent<Button>().onClick.AddListener(() => { background_img_viewPort.gameObject.SetActive(true); Load_Image_review(_img_name_str, 6); });//6th is the full screen image
